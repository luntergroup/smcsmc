...
plot.smcsmc <- function( data = NULL, truth = NULL, g=30 ) {
...
##    p <- p + scale_y_log10(limits=c(1000,nemax),breaks=c(2000,5000,10000,20000,50000,100000))
    p <- p + coord_trans(x="log10",y="log10", limx=c(t0,t1),limy=c(1000,nemax * 0.97))
    p <- p + scale_x_continuous(breaks=c(1000,10000,100000,1000000),labels=c("1000","10,000","100,000","1,000,000"))
