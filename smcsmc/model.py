...
class Smcsmc:
    def __init__(self, opts):
        ...
        self.alpha = (
            0.0  # posterior mix-in; 0 means use prior; <0 means remove .recomb.gz files
        )
        self.beta = 4  # smoothness parameter; see processrecombination.py
        self.maxNE = 1e99
        self.sampletrees = False
        self._processes = []
        self.threads = True
        self.cluster = False
        self.cconfig = None
        ...
        self.smcsmcpath = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../smcsmc")
        )
        ...
        self.options = [
            ...
            (
                ...
                "-t",
                "th",
                "[+] Set mutation rate (expected number of mutations) for the locus (=4 N0 mu L )",
            ),
            ...
            (
                ...
                "-I",
                "n s1..sn",
                "Use an n-population island model with si individuals sampled",
            ),
            (
                0,
                "-ej",
                "t i j",
                "Speciation event at t*4N0; creates population i from population j in forward direction",
            ),
            (
                0,
                "-eI",
                "t s1..sn",
                "Sample s1..sn indiviuals from their populations at time t*4N0 generations",
            ),
            (
                1,
                "-rho",
                "rh",
                "[+] Set recombination rate (per nucleotide per generation)",
            ),
            (
                1,
                "-r",
                "r L",
                "[+] Set initial per locus recombination rate (=4 N0 L rho) and locus length (L)",
            ),
            (
                1,
                "-eN",
                "t n",
                "Change the size of all populations to n*N0 at time t*4N0",
            ),
            (
                1,
                "-en",
                "t i n",
                "Change the size of population i to n*N0 at time t*4N0",
            ),
            (
                1,
                "-eM",
                "t m",
                "Change the symmetric backward migration rate to m/(npop-1) at time t*4N0",
            ),
            (
                1,
                "-em",
                "t i j m",
                "Change the backward migration rate, from population i to population j, to m/(npop-1) at time t*4N0",
            ),
            (
                1,
                "-ema",
                "t s11 s12 ...",
                "Set backward migration rate matrix at time t*4N0",
            ),
            (2, "-o", "f", "[*] Output prefix"),
            ...
            (
                ...
                "-segs",
                "f1 f2 ...",
                "[+] Input .seg files (will be merged into a single .seg file",
            ),
            (
                2,
                "-maxgap",
                "n",
                "Split .seg files over gaps larger than maxgap (200 kb)",
            ),
            (
                2,
                "-minseg",
                "n",
                "After splitting ignore segments shorter than minseg (500 kb)",
            ),
            ...
            (
                ...
                "-P",
                "s e p",
                "Divide time interval [s,e] (generations; s>0) equally on log scale, using pattern p (e.g. 1*2+8*1)",
            ),
            ...
            (
                ...
                "-calibrate_lag",
                "s",
                "Accumulate inferred events with a lag of  s  times the survival time (2)",
            ),
            (
                2,
                "-apf",
                "b",
                "Auxiliary particle filter: none (0), singletons (1), cherries (2)  (0)",
            ),
            (
                2,
                "-dephase",
                "",
                "Dephase heterozygous sites (but use phasing for -apf)",
            ),
            ...
            (
                ...
                "-bias_heights",
                "t0..tn",
                "Set recombination bias times to h0..hn * 4N0",
            ),
            (2, "-bias_strengths", "s1..sn", "Set recombination bias strenghts"),
            (2, "-arg", "", "Sample posterior ARG; produce .trees file"),
            (
                3,
                "-EM",
                "n",
                "Number of EM (or VB) iterations-1 ({})".format(self.emiters),
            ),
            (
                3,
                "-VB",
                "",
                "Use Variational Bayes rather than EM (uniform prior for all rates)",
            ),
            ...
            (
                ...
                "-chunks",
                "n",
                "Number of chunks computed in parallel ({})".format(self.chunks),
            ),
            ...
            (
                ...
                "-no_m_step",
                "",
                "Do not update parameters (but do infer recombination guide)",
            ),
            (
                3,
                "-alpha",
                "t",
                "Fraction of posterior recombination to mix in to recombination guide ({}); negative removes files".format(
                    self.alpha
                ),
            ),
            (
                ...
                "-c",
                "",
                "Use qsub to submit job(s) to cluster (overrides use of threads)",
            ),
            (
                3,
                "-C",
                "opts",
                'Qsub config parameter(s) e.g. "-P project.prj -q long.q"; overrides ./qsub.conf',
            ),
            (
                3,
                "-nothreads",
                "",
                "Calculate chunks sequentially rather than in parallel threads",
            ),
            (
                3,
                "-smcsmcpath",
                "f",
                "Path to smcsmc executable ({})".format(self.smcsmcpath),
            ),
    ...
    def parse_opts(self):
        ...
        while idx < len(opts):
            ...
            if opts[idx] == "-t":
                ...
                raise ValueError(
                    "SCRM option '{}' is illegal in SMCSMC; for options -m -M -n use alternatives -em -eM -en".format(
                        opts[idx]
                    )
                )
            else:
        ...
        if self.cluster:
            if self.cconfig is None:
                try:
                    ...
                    self.cconfig = ""
            smcsmc.execute.qsub_config = (
                self.cconfig
            )  # assign to global variable within module 'execute'

    def set_pattern(self):
        if self.patt_args is None:
            return
        if len(self.patt_args) != 3:
            raise ValueError(
                "-P should get 3 arguments (start and end generation; and pattern)"
            )
        start, end = map(float, self.patt_args[:2])
        if start <= 0:
            raise ValueError("-P: start generation should be > 0")
        ...
        for ii, idx in enumerate(opt_indices[:-1]):
            if opts[idx] in ("-eI", "-ej", "-eM", "-ema", "-em", "-eN", "-en"):
                time_opts.append(
                    (float(opts[idx + 1]), opts[idx : opt_indices[ii + 1]])
                )
            else:
        ...
        times = [0] + [
            start
            * math.exp(math.log(end / start) * (i - 1) / (len(mask) - 2.0))
            / (4 * self.N0)
        ...
        for idx, time in enumerate(times):
            if mask[idx] == 1:
                ...
                last_en_opt = sorted(
                    [(-1.0, ["-eN", "0.0", "1.0"])]
                    + [to for to in time_opts if to[0] <= time and to[1][0] == "-eN"]
                )[-1]
                new_time_opts.append(
                    (time, ["-eN {} {}".format(time, last_en_opt[1][2])])
                )
        ...
        logger.info(
            "Population structure options: {}".format(
                " ".join([" ".join(opt) for t, opt in new_time_opts])
            )
        )
    ...
    def read_segline(self, line):
        ...
        raise ValueError(
            "Error parsing a .seg line (expected 3 or 6 fields): '{}'".format(
                " ".join(elts)
            )
        )
    ...
    def define_chunks(self):
        ...
        for line in self.open_file(self.segfile, "r"):
            ...
            if harmonic is None:
                harmonic = [
                    sum(1.0 / (i) for i in range(1, k + 1)) for k in range(alleles)
                ]
        ...
        if self.length is None:
            ...
            if self.length > last - self.startpos:
                if self.length > last - self.startpos + 1000000:
                    raise ValueError(
                        "Asked to process {} bp, but .seg file(s) provided only cover positions {}-{} ({} bp)".format(
                            self.length, self.startpos, last, last - self.startpos
                        )
                    )
                else:
                    logger.info(
                        "Warning: asked to process {} bp, but .seg file(s) provided only cover positions {}-{} ({} bp)".format(
                            self.length, self.startpos, last, last - self.startpos
                        )
                    )
        smallchunks = [
            i
            for i in range(len(chunkstarts) // 2)
            if chunkstarts[2 * i + 1] - chunkstarts[2 * i] < self.minseg
        ]
        smallchunklen = sum(
            chunkstarts[2 * i + 1] - chunkstarts[2 * i] for i in smallchunks
        )
        ...
        logger.info(
            "Found {} segments in data separated by gaps larger than {} bp (including chromosome boundaries)".format(
                len(chunkcounts), self.maxgap
            )
        )
        logger.info(
            "Skipped {} small segments covering {} bp".format(
                len(smallchunks), smallchunklen
            )
        )
        if len(chunkcounts) > self.chunks:
            logger.info(
                "Note: using more chunks ({}) than asked for ({}); to reduce, decrease the number of chromosomes and/or increase -maxgap".format(
        ...
        while sum(chunkcounts) < self.chunks:
            chunksizes = [
                (chunkstarts[2 * i + 1] - chunkstarts[2 * i]) / c
        ...
        for i, c in enumerate(chunkcounts):
            for ii in range(c):
                ...
                logger.info(
                    " Chunk {}: start {} length {}".format(
                        len(self.chunkstarts) - 1,
                        self.chunkstarts[-1],
    ...
    def validate(self):
        if self.segfile == None:
            raise ValueError("Option -seg or -segs required")
        if self.outprefix == None:
            raise ValueError("Option -o required")
        if not os.path.exists(self.smcsmcpath):
            raise ValueError(
                "Can't find executable " + self.smcsmcpath + "; check -smcsmcpath"
            )
        self.prepare_folder(0, log=False)
        ...
        logger.info(
            "Options passed to populationmodels: '{}'".format(" ".join(self.opts))
        )
    ...
    def validate_parameters(self):
        ...
        if self.mutation_rate == None:
            if self.theta != None and self.N0 != None:
                logger.info(
                    "Setting mutation rate (per nt per generation) from -t and N0"
                )
        ...
        if self.recombination_rate == None:
            if self.rho != None and self.N0 != None:
                logger.info(
                    "Setting recombination rate (per nt per generation) from -t and N0"
                )
        ...
        if self.N0 == None:
            if self.mutation_rate != None:
                logger.info(
                    "Setting N0 from mutation rate and Watterson's estimate of theta"
                )
        ...
        if self.rho == None:
            if self.N0 != None:
                logger.info(
                    "Setting -r parameter from N0, recombination rate, and length"
                )
                self.rho = 4 * self.N0 * self.recombination_rate * self.length

        if self.N0 == None:
            raise ValueError("N0 required -- use -N0, or (implicitly) -mu")
        if self.mutation_rate == None:
            raise ValueError(
                "Mutation rate required -- use -mu, or (implicitly) -t / -N0"
            )
        if self.recombination_rate == None:
            raise ValueError(
                "Recombination rate required -- use -rho, or (implicitly) -r / -N0"
            )
        if self.theta == None:
            raise ValueError("-t value required -- use -t or (implicitly) -mu / -N0")
        if self.rho == None:
            raise ValueError("-r value required -- use -r or (implicitly) -rho / -N0")
        if self.length == None:
            raise ValueError("Option -r required")

        # ensure parameters are consistent
        if (
            abs(4 * self.N0 * self.mutation_rate * self.length / self.theta - 1.0)
            > 1e-5
        ):
            raise ValueError("Values for -t and -N0, -mu, -length not consistent")
        if (
            abs(4 * self.N0 * self.recombination_rate * self.length / self.rho - 1.0)
            > 1e-5
        ):
            raise ValueError("Values for -r and -N0, -rho, -length not consistent")

        if self.emiters < 0:
            raise ValueError("-EM set to {}; must be >=0".format(self.emiters))
        if self.length < 1:
            raise ValueError("Locus length ({}) must be >=1".format(self.length))
        if self.chunks < 1:
            raise ValueError("Number of chunks ({}) must be >=1".format(self.chunks))
        ...
        logger.info(
            "  theta (4 N0 mu):      {}".format(4 * self.N0 * self.mutation_rate)
        )
        logger.info("  Watterson's estimate: {}".format(self.watterson_estimator))
        logger.info("  locus size:           {}".format(self.length))
        logger.info(
            "Running inference over {} bp in {} chunks and {} EM iterations".format(
                self.length, len(self.chunkstarts), self.emiters + 1
            )
        )
        logger.info(
            "Unique chunk sizes (ordered): {}".format(
                " ".join(
                    map(
                        str,
                        sorted(
                            list(
                                set(
                                    (
                                        self.chunkends[i] - start
                                        for i, start in enumerate(self.chunkstarts)
                                    )
                                )
                            )
                        ),
    ...
    def parse_segfiles(self, idx0, files):
        if self.segfile != None:
            raise ValueError("You can only provide the --seg or --segs option once!")
        segfiles = []
    ...
    def prepare_folder(self, iteration, log=True):
        ...
        if log:
            logger.info(
                "Preparing directory {} for EM iteration {}".format(dirname, iteration)
            )
        if not os.path.isdir(dirname):
            try:
                ...
                raise ValueError(
                    "Cannot find or create directory '{}'".format(
    ...
    def parse_outfile(self, outfname, data=None, clump=None):
        ...
        for line in of:
            ...
            for name, val in listitems(
                elts
            ):  # Was list(elts.iteritems(); 2.x/3.x portability.  [Leave "Type" as String]
            ...
            if len(iters) > 1:
                raise ValueError(
                    "Found multiple iterations in .out file {}; expected only one".format(
                        outfname
                    )
                )
            ...
            if clump != None:
                keys.append(
                    (elts["Type"], elts["Epoch"], elts["From"], elts["To"], clump)
                )
            for key in keys:
    ...
    def write_outfile(self, outfname, data, iteration):
        ...
        for key in sorted(data.keys(), key=lambda elt: (elt[0][-1] >= 0, elt)):
            if key[1] == "Count":
                start, end, opp, count, wt = map(
                    lambda label: data[(key[0], label)],
                    ["Start", "End", "Opp", "Count", "Wt"],
                )
                typ, epoch, frm, to, clump = key[0]
                if typ == "LogL":
                    opp, wt = 1.0, 1.0  # just add log likelihoods, don't average them
                rate = count / (opp + 1e-30)
                ne = (opp + 1e-10) / (2.0 * count + 1e-30) if typ == "Coal" else 0.0
                ess = 1.0 / (wt / (opp + 1e-30))
                of.write(
                    "{:6d} {:>6d} {:11.5g} {:11.5g} {:>6s}  {:>5d}  {:>5d} {:14.8g} {:14.8g} {:14.8g} {:14.8g} {:11.5g} {:>6d}\n".format(
                        iteration,
                        epoch,
                        start,
                        end,
    ...
    def merge_outfiles(self):
        ...
        write_header = True
        for em_iter in range(self.emiters, -1, -1):
    ...
    def m_step(self, data):
        ...
        for epoch in range(len(self.pop.change_points)):
            for pop in range(self.pop.num_populations):
                ...
                rate = count / opp
                popsize = min(
                    self.maxNE / self.pop.N0, 1.0 / (2.0 * rate * self.pop.N0)
                )
                self.pop.population_sizes[epoch][pop] = popsize
                self.pop.population_event_counts[epoch][pop] = count
                logger.info(
                    "Setting population size (epoch {} population {}) to {}   ({} coalescence events)".format(
                        epoch, pop, popsize, count
                    )
                )
        # set migration rates
        for epoch in range(len(self.pop.change_points)):
            for frompop in range(self.pop.num_populations):
                for topop in range(self.pop.num_populations):
                    if frompop != topop:
                        ...
                        self.pop.migration_rates[epoch][frompop][topop] = (
                            rate * 4 * self.pop.N0
                        )
                        self.pop.migration_event_counts[epoch][frompop][topop] = count
                        logger.info(
                            "Setting migration rate (epoch {} population {}->{}) to {}  ({})  ({} migration events)".format(
                                epoch,
                                frompop,
                                topop,
                                rate,
    ...
    def e_step(self, iteration, chunk):
        ...
        command += [
            "-nsam",
            str(len(self.pop.sample_populations)),
            "-startpos",
            str(self.pop.startpos),
            "-EM 0",
            "-log",
            "-o",
            self.template.format(iteration, chunk),
            ">",
            self.stdout_template.format(iteration, chunk),
            "2>",
            self.stderr_template.format(iteration, chunk),
        ]
        ...
        if self.have_outfile(iteration, chunk):
            logger.info(
                "Results for iteration {} chunk {} already present -- continuing".format(
                    iteration + 1, chunk
                )
            )
        ...
        if self.threads and not self.cluster:
            ...
            self._processes.append(
                p
            )  # add to list of processes, so that we can wait for them to be done
        else:
            smcsmc.execute.submit(command, shell=True)
    ...
    def do_iteration(self, iteration):
        ...
        if self.have_outfile(iteration, "final"):
            logger.info(
                "Results for iteration {} already exist -- continuing".format(
                    iteration + 1
                )
            )
            if iteration == self.emiters:
                logger.info(
                    "All iterations were already present -- results not recomputed"
                )
            return

        # build default population class.  Just need to set sequence_length, and we're good to go
        self.pop = smcsmc.populationmodels.Population()
        ...
        if iteration > 0:
            self.m_step(
                self.parse_outfile(self.out_template.format(iteration - 1, "final"))
            )
            if self.alpha > 0.0:
                ...
                for chunk in range(len(self.chunkstarts)):
                    lr = processrecombination.LocalRecombination(
                        self.recomb_template.format(iteration - 1, chunk)
                    )
                    lr.smooth(self.alpha, self.beta)
                    self.recombination_guide = self.recomb_guide_template.format(
                        iteration, chunk
                    )
        ...
        while len(self._processes) > 0:
            logger.info(
                "Waiting for child process {} to finish...".format(len(self._processes))
            )
            returncode = self._processes[-1].wait()
            if returncode != 0:
                raise ValueError(
                    "smcsmc executable for iteration {}, chunk {}, returned non-zero exit code".format(
        ...
        data = None
        for chunk in range(len(self.chunkstarts)):
            data = self.parse_outfile(
                self.out_template.format(iteration, chunk), data, chunk
            )
        self.write_outfile(
            self.out_template.format(iteration, "final"), data, iteration
        )
