...
class Population:
    def __init__(
        ...
        change_points=[0, 0.01, 0.06, 0.2, 1, 2],
        ...
        self.population_sizes = (
            population_sizes  # list, or list of lists (per population)
        )
        self.migration_rates = migration_rates  # None, or list of lists of lists
        self.population_event_counts = (
            None  # filled with dummy values for 1st iteration; updated by hand later
        )
        self.migration_event_counts = (
            None  # filled with dummy values for 1st iteration; updated by hand later
        )
        self.num_populations = num_populations
        self.sample_populations = (
            sample_populations  # encoded as sample_populations_ in model.h
        )
        ...
        while idx < len(opts):
            if opts[idx] == "-N0":
                ...
                if len(self.change_points) != 0:
                    raise ValueError(
                        "Options -I must go before -eN / -en / -eM / -em / -ema"
                    )
                ...
                self.migration_commands[
                    len(self.change_points) - 1
                ] += " -ej {} {} {}".format(self.change_points[-1], source, sink)
                ...
                idx += 1
        if self.sample_populations == []:
            self.sample_populations = None
    ...
    def _parse_time(self, opt_idx, opts):
        if self.change_points == []:
            ...
            self.migration_rates = [
                [[0.0] * self.num_populations for _ in range(self.num_populations)]
            ]
    ...
    def _finalize_and_validate(self):
        ...
        for idx, migr_rates in enumerate(self.migration_rates):
            if migr_rates is None:
                migr_rates = [
                    [0] * self.num_populations for dummy in range(self.num_populations)
                ]
        ...
        if type(self.population_sizes[0]) != list:
            self.population_sizes = [
                [size] * self.num_populations for size in self.population_sizes
            ]
        for idx, migr_rates in enumerate(self.migration_rates):
        ...
        if self.population_event_counts is None:
            self.population_event_counts = [
                [1e10] * self.num_populations for popsizes in self.population_sizes
            ]
        if self.migration_event_counts is None:
            self.migration_event_counts = [
                [[1e10 for rate in migr_vec] for migr_vec in migr_mat]
                for migr_mat in self.migration_rates
            ]
    ...
    def _create_sample_command_line_options(self):
        ...
        for time in times:
            ...
            if time == 0.0:
                expression = "-I {} {}".format(
                    self.num_populations, " ".join(map(str, popsizes))
                )
            else:
                expression = expression + " -eI {} {}".format(
                    time, " ".join(map(str, popsizes))
                )
        return expression

    def _create_popsize_migration_command_line_options(
        self,
        ...
        self._finalize_and_validate()
        my_popsizes = (
            self.population_sizes if inference_popsizes is None else inference_popsizes
        )
        my_migrationrates = (
            self.migration_rates
            if inference_migrationrates is None
            else inference_migrationrates
        )
        my_changepoints = (
            self.change_points
            if inference_changepoints is None
            else inference_changepoints
        )
        my_migrationcommands = (
            self.migration_commands
            if inference_migrationcommands is None
            else inference_migrationcommands
        )

        expression = []
        if vb:
            expression.append("-vb")
        for i in range(len(my_changepoints)):
            ...
            if len(set(popsizes)) == 1:
                if vb:
                    expression.append(
                        "-eN {} {} {}".format(
                            time, popsizes[0], self.population_event_counts[i][0]
                        )
                    )
                else:
                    ...
                    if vb:
                        expression.append(
                            "-en {} {} {} {}".format(
                                ...
                                idx + 1,
                                popsize,
                                self.population_event_counts[i][idx],
                            )
                        )
                    else:
            ...
            all_rates = set(
                migration_matrix[j][k]
                for j, k in itertools.product(
                    range(self.num_populations), range(self.num_populations)
                )
                if j != k
            )
            if len(all_rates) == 1:
                ...
                total_event_count = sum(
                    self.migration_event_counts[i][j][k]
                    for j, k in itertools.product(
                        range(self.num_populations), range(self.num_populations)
                    )
                )
                if vb:
                    expression.append(
                        "-eM {} {} {}".format(
                            time, total_symmetric_rate, total_event_count
                        )
                    )
                else:
    ...
    def core_command_line(
        ...
        self.recombinations = (
            4 * self.N0 * self.recombination_rate * self.sequence_length
        )
    ...
    def simulate(self, missing_leaves=[], phased=True, debug=False):
        ...
        if self.filename == None:
            final_filename = command.replace(" ", "_") + ".seg"
        else:
            final_filename = self.filename

        temp_file = tempfile.NamedTemporaryFile(
            dir=os.path.dirname(os.path.abspath(final_filename))
        )
        ...
        command = (
            "{cmd} {nsam} {nloci} {core} {seed} -T -L -p 10 -l 300000 > {fn}".format(
                cmd=self.scrmpath,
    ...
    def convert_scrm_to_seg(self, infilename, outfilename, missing_leaves, phased):
        ...
        for line in scrmfile:
            if line.startswith("positions:"):
                ...
                if positions == None:
                    raise ValueError(
                        "Unexpected output from scrm -- no 'positions' line found"
                    )
                dataline = line.strip()
                ...
                if len(data[-1]) != len(positions):
                    raise ValueError(
                        "Unexpected number of mutations: got {}, expected {}".format(
        ...
        positions = list(
            map(lambda realpos: int(realpos * self.sequence_length + 0.5), positions)
        )
    ...
    def convert_scrm_to_recomb(
        self, infilename, outfilename, segsize=100, fourne=40000
    ):
        ...
        if len(recombinations) != len(treelengths):
            raise ValueError(
                "Unexpected number of tree lengths; got {}, expected {}".format(
                    len(treelengths), len(recombinations)
                )
            )
        scrmfile.close()
                    ...
                    while True:
                        ...
                        while pos + self.segsize < newpos + length:
                            ...
                            if newpos < pos + self.segsize:
                                integral += height * (
                                    pos + self.segsize - max(pos, newpos)
                                )
                        ...
                        integral += (
                            max(
                                0,
                                min(pos + self.segsize, newpos + length)
                                - max(pos, newpos),
                            )
                            * height
                        )
        ...
        for pos, length, opp, rec in collapsed:
            if type(opp) == float:
                opp = "{:11.2f}".format(opp)
            outfile.write(
                "{pos}\t{length}\t{opp}\t{rec}\n".format(
                    pos=pos, length=length, opp=opp, rec=rec
                )
            )
        outfile.close()
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--model",
        dest="model",
        help="choose model {}".format(labels),
        default="Population",
    )
    parser.add_option(
        "-n",
    ...
    parser.add_option(
        "-l", "--seqlen", type="int", dest="seqlen", help="sequence length", default=1e6
    )
    parser.add_option(
        "-s", "--seed", type="int", dest="seed", help="random number seed", default=1
    )
