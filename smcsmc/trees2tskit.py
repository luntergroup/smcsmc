...
#
def eventRepr(event):
    return "{:7.1f} {} {:10.1f} {: 1}->{: 1}  {}".format(
        event.height,
        event.type,
        float(event.pos),
        event.frm,
        event.to,
        "".join(reversed("{:b}".format(event.descendants))),
    )
...
def update(tree, event, start_height, lineage):
    ...
    for anc_idx in range(idx, len(tree)):
        if lineage & tree[anc_idx].descendants:
            check(
                lineage & tree[anc_idx].descendants == lineage,
                "Inconsistent tree structure",
            )
            tree[anc_idx] = setDescendants(
                tree[anc_idx],
                (tree[anc_idx].descendants & ~lineage) | RECOMBINATION_ANCESTOR,
            )
    ...
    while idx < len(tree) and tree[idx].height < event.height:
        idx += 1
    check(
        idx >= len(tree) or tree[idx].height > event.height, "duplicate heights found"
    )
    ...
    while idx < len(tree):
        ...
        tree[idx] = setDescendants(
            tree[idx], tree[idx].descendants & ~RECOMBINATION_ANCESTOR
        )
        idx += 1
    return tree
...
#
def update_edges_and_migrations(
    current_edges,
    tree,
    edgelist,
    current_pos,
    ...
    for edge in current_edges:
        edgelist[current_edges[edge]] = setRight(
            edgelist[current_edges[edge]], current_pos
        )
    ## done
    ...
    global back_coals
    if d:
        print("ending " + str(len(current_migrations.keys())) + " segments")
...
def trees2tskit(infile, hap=2, suffix=".trees.gz", write_all=False, d=False):
    ...
    while idx < len(data):
        ...
        while (
            end_idx < len(data)
            and data[end_idx].type != "R"
            and data[end_idx - 1].type != "C"
            and data[end_idx].pos == data[end_idx - 1].pos
        ):
            end_idx = end_idx + 1
        ...
        if end_idx < len(data) and data[end_idx].type == "R":
            ...
            if STORE_RECOMBINATION:
                ...
                nodelist[data[end_idx].nodeid] = setPopulation(
                    nodelist[data[end_idx].nodeid],
                    nodelist[data[idx].nodeid].population,
                )
            ...
            start_height = 0
            lineage = 2 ** (
                len(bin(data[end_idx - 1].descendants)) - 3
            )  ## rightmost lineage of those participating in coalescent
        ...
        for event in data[idx:end_idx]:
            ...
            check(
                ...
                "****** Recombination occurred above coalescent or migration",
            )
            check(
                lineage & event.descendants == lineage,
                "****** Inconsistent tree structure",
            )
        ...
        current_edges, migration_list, segments = update_edges_and_migrations(
            current_edges,
            tree,
            edgelist,
            data[next_idx - 1].pos,
