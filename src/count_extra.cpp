...
void CountModel::extract_and_update_count(ParticleContainer &Endparticles, double current_base, bool end_data ) {
    ...
    for (size_t epoch_idx = 0; epoch_idx < this->change_times_.size(); epoch_idx++) {
		...
		for (size_t i = 0; i < Endparticles.particles.size(); i++) {
            ...
            this->update_coalescent_count( thisState->eventContainer[ epoch_idx ], thisState->CoaleventContainer[ epoch_idx ], weight, x_end, this->total_coal_count[ epoch_idx ],   this->total_weighted_coal_opportunity[ epoch_idx ], epoch_idx );
            if (this->population_number() > 1){
                this->update_migration_count( thisState->eventContainer[ epoch_idx ], thisState->MigreventContainer[ epoch_idx ], weight, x_end, epoch_idx );
            }
            this->update_recombination_count( thisState->eventContainer[ epoch_idx ], thisState->RecombeventContainer[ epoch_idx ], weight, this->counted_to[epoch_idx], x_end, this->total_recomb_count[ epoch_idx ], this->total_weighted_recomb_opportunity[ epoch_idx ], epoch_idx );
        }
