...
calculate.quartiles <- function( frame, field="Ne", miny=1000, maxy=100000, mint=30, maxt=10000, scale=1, add=0 ) {
    ...
    keys <- unique(data.frame(frame$type, frame$frm, frame$epoch, frame$aux_part_filt, frame$int_parameter, frame$np, frame$iter))
    for (i in 1:nrow(keys)) {
        data <- subset(frame, type==keys[i,1] & frm==keys[i,2] & epoch==keys[i,3] &
                              aux_part_filt==keys[i,4] & int_parameter==keys[i,5] & np==keys[i,6] & iter==keys[i,7])
